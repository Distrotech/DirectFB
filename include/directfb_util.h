...
D_LOG_DOMAIN( DirectFB_Util, "DirectFB/Util", "DirectFB Util" );
...
#define DFB_RECTANGLES2_DEBUG_AT( Domain, rects, rects2, num )                                                \
     do {                                                                                                     \
          unsigned int i;                                                                                     \
                                                                                                              \
          for (i=0; i<(num); i++)                                                                             \
               D_DEBUG_AT( Domain, "  -> [%2d] %4d,%4d-%4dx%4d <- %4d,%4d-%4dx%4d\n", i,                      \
...
#define DFB_RECTANGLES_POINTS_DEBUG_AT( Domain, rects, points, num )                                          \
     do {                                                                                                     \
          unsigned int i;                                                                                     \
                                                                                                              \
          for (i=0; i<(num); i++)                                                                             \
               D_DEBUG_AT( Domain, "  -> [%2d] %4d,%4d-%4dx%4d <- %4d,%4d\n", i,                              \
...
#define DFB_TRIANGLES_DEBUG_AT( Domain, tris, num )                                                           \
     do {                                                                                                     \
          unsigned int i;                                                                                     \
                                                                                                              \
          for (i=0; i<(num); i++)                                                                             \
               D_DEBUG_AT( Domain, "  -> [%2d] %4d,%4d-%4d,%4d-%4d,%4d\n", i, DFB_TRIANGLE_VALS(&(tris)[i]) );\
     } while (0)
...
#if D_DEBUG_ENABLED
#define DFB_UPDATES_ASSERT(_Up)                                                                          \
     do {                                                                                                \
          D_MAGIC_ASSERT( _Up, DFBUpdates );                                                             \
          D_ASSERT( (_Up)->regions != NULL );                                                            \
          D_ASSERT( (_Up)->max_regions > 0 );                                                            \
          D_ASSERT( (_Up)->num_regions <= (_Up)->max_regions );                                          \
     } while (0)

#define DFB_UPDATES_LOG(Domain, LEVEL, _Up)                                                                \
     do {                                                                                                \
          D_LOG( Domain, LEVEL, "  -> %d/%d regions\n", (_Up)->num_regions, (_Up)->max_regions );          \
                                                                                                         \
          if ((_Up)->num_regions) {                                                                      \
               D_LOG( Domain, LEVEL, "  -> [[ ]] %4d,%4d-%4dx%4d\n",                                       \
                           DFB_RECTANGLE_VALS_FROM_REGION(&(_Up)->bounding) );                           \
                                                                                                         \
               if ((_Up)->num_regions > 1)                                                               \
                    DFB_REGIONS_LOG( Domain, LEVEL, (_Up)->regions, (_Up)->num_regions );                  \
          }                                                                                              \
     } while (0)

#define DFB_UPDATES_DEBUG_AT(Domain, _Up)                                                                \
     DFB_UPDATES_LOG(Domain, DEBUG, _Up)
#else
#define DFB_UPDATES_ASSERT(_Up)                                                                          \
     do {                                                                                                \
     } while (0)
...
void       dfb_updates_init_from     ( DFBUpdates       *updates,
                                       DFBRegion        *regions,
                                       unsigned int      max_regions,
                                       const DFBUpdates *source );

DFBRegion *dfb_updates_add           ( DFBUpdates      *updates,
                                       const DFBRegion *region );

void       dfb_updates_stat          ( const DFBUpdates *updates,
                                       int              *ret_total,
                                       int              *ret_bounding );

void       dfb_updates_get_rectangles( const DFBUpdates *updates,
                                       DFBRectangle     *ret_rects,
                                       int              *ret_num );

void       dfb_updates_get_boxes     ( const DFBUpdates *updates,
                                       DFBBox           *ret_boxes,
                                       int              *ret_num );

int        dfb_updates_get_pixels    ( const DFBUpdates *updates );

bool       dfb_updates_intersects    ( const DFBUpdates *updates,
                                       const DFBBox     *box );

void       dfb_updates_translate     ( DFBUpdates       *updates,
                                       const DFBPoint   *offset );

/**********************************************************************************************************************/

static inline void
dfb_updates_add_box( DFBUpdates   *updates,
                     const DFBBox *box )
{
     if (!DFB_BOX_EMPTY( box )) {
          DFBRegion region = DFB_REGION_INIT_FROM_BOX( box );
...
static inline void
dfb_updates_add_boxes( DFBUpdates   *updates,
                       const DFBBox *boxes,
                       unsigned int  num )
{
     int i;
     ...
     for (i=0; i<num; i++)
          dfb_updates_add_box( updates, &boxes[i] );
}

static inline void
dfb_updates_add_rects( DFBUpdates         *updates,
                       ...
                       unsigned int        num )
{
     ...
     for (i=0; i<num; i++) {
          DFBRegion region = DFB_REGION_INIT_FROM_RECTANGLE( &rects[i] );
...
static inline void
dfb_updates_add_region( DFBUpdates *updates,
                        int         x1,
                        int         y1,
                        int         x2,
                        int         y2 )
...
void DFBTachopese_Init( DFBTachopese *tachopese );

void DFBTachopese_SetInterval( DFBTachopese *tachopese, int millis );

void DFBTachopese_Start( DFBTachopese *tachopese );
bool DFBTachopese_Continue( DFBTachopese *tachopese );
void DFBTachopese_Stop( DFBTachopese *tachopese );

float DFBTachopese_GetStringsPerSecond( DFBTachopese *tachopese );
float DFBTachopese_GetCharsPerSecond( DFBTachopese *tachopese );
float DFBTachopese_GetMovesPerSecond( DFBTachopese *tachopese );
float DFBTachopese_GetFlipsPerSecond( DFBTachopese *tachopese );
float DFBTachopese_GetFlipselsPerSecond( DFBTachopese *tachopese );
float DFBTachopese_GetPixelsPerSecond( DFBTachopese *tachopese );
float DFBTachopese_GetSeconds( DFBTachopese *tachopese );
float DFBTachopese_GetUserSeconds( DFBTachopese *tachopese );
float DFBTachopese_GetSystemSeconds( DFBTachopese *tachopese );
float DFBTachopese_GetCPUSeconds( DFBTachopese *tachopese );
float DFBTachopese_GetUserUsage( DFBTachopese *tachopese );
float DFBTachopese_GetSystemUsage( DFBTachopese *tachopese );
float DFBTachopese_GetCPUUsage( DFBTachopese *tachopese );

unsigned long long DFBTachopese_GetFlips( DFBTachopese *tachopese );
unsigned long long DFBTachopese_GetFlipsels( DFBTachopese *tachopese );
unsigned long long DFBTachopese_GetPixels( DFBTachopese *tachopese );

void DFBTachopese_Move( DFBTachopese *tachopese, int dx, int dy );
void DFBTachopese_Flip( DFBTachopese *tachopese, int width, int height );
void DFBTachopese_DrawString( DFBTachopese *tachopese, int length );
void DFBTachopese_FillRectangle( DFBTachopese *tachopese, int width, int height );
void DFBTachopese_FillRectangles( DFBTachopese *tachopese, const DFBRectangle *rects, int num );
void DFBTachopese_DrawRectangle( DFBTachopese *tachopese, int width, int height );
void DFBTachopese_DrawLine( DFBTachopese *tachopese, int x1, int y1, int x2, int y2 );
