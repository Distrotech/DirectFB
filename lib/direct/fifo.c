...
direct_fifo_push( DirectFifo *fifo, DirectFifoItem *item )
{
     ...
     index = D_SYNC_ADD_AND_FETCH( &fifo->count, 1 ) - 1;
     D_ASSERT( index >= 0 );
...
#endif
          if (fifo->waiting && fifo->count) {
               direct_futex_wake( &fifo->count, 1 );
...
direct_fifo_pull( DirectFifo *fifo )
{
     ...
     if (tmp) {
          ...
          tmp = D_SYNC_FETCH_AND_CLEAR( &fifo->in );
          if (!tmp)
               return NULL;
     ...
     index = D_SYNC_ADD_AND_FETCH( &fifo->count, -1 );
...
direct_fifo_wakeup( DirectFifo *fifo )
{
     ...
     return direct_futex_wake( &fifo->count, 1 );
}
