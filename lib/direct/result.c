...
static DirectHash result_types = DIRECT_HASH_INIT( 23, true );

/**********************************************************************************************************************/

DirectResult
DirectResultTypeRegister( const DirectResultType *type )
{
     ...
     return direct_hash_insert( &result_types, type->base, (void*) type );
}

DirectResult
DirectResultTypeUnregister( const DirectResultType *type )
{
     D_DEBUG_AT( Direct_Result, "%s( %p )\n", __FUNCTION__, type );
     ...
     return direct_hash_remove( &result_types, type->base );
}
