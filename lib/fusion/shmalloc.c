...
fusion_dbg_print_memleaks( FusionSHMPoolShared *pool )
{
     ...
     if (pool->allocs) {
          ...
          direct_list_foreach (desc, pool->allocs) {
               direct_log_printf( NULL, " %9lu bytes at %p [%8lu] in %-30s [%3lx] (%s: %u)\n",
                     ...
                     const char *func, size_t __size )
{
     ...
     if (ret) {
          D_DERROR( ret, "Fusion/SHM: Could not allocate %lu bytes from pool!\n", __size + sizeof(SHMemDesc) );
          fusion_skirmish_dismiss( &pool->lock );
          return NULL;
     }
     ...
     D_DEBUG_AT( Fusion_SHM, "allocating %9lu bytes at %p [%8lu] in %-30s [%3lx] (%s: %u)\n",
                 desc->bytes, desc->mem, (ulong)desc->mem - (ulong)pool->heap,
                 desc->func, desc->fid, desc->file, desc->line );
                      ...
                      size_t __size )
{
     ...
     if (ret) {
          D_DERROR( ret, "Fusion/SHM: Could not reallocate from %lu to %lu bytes!\n",
     ...
     D_DEBUG_AT( Fusion_SHM, "reallocating %9lu bytes at %p [%8lu] in %-30s [%3lx] (%s: %u) '%s'\n",
                 desc->bytes, desc->mem, (ulong)desc->mem - (ulong)pool->heap,
                   ...
                   const char *func, const char *what, void *__ptr )
{
     ...
     D_DEBUG_AT( Fusion_SHM, "freeing %9lu bytes at %p [%8lu] in %-30s [%3lx] (%s: %u) '%s'\n",
                 desc->bytes, desc->mem, (ulong)desc->mem - (ulong)pool->heap,
                 desc->func, desc->fid, desc->file, desc->line, what );
                     ...
                     const char *func, const char *string )
{
     ...
     D_DEBUG_AT( Fusion_SHM, "allocating %9lu bytes at %p [%8lu] in %-30s [%3lx] (%s: %u) <- \"%s\"\n",
                 desc->bytes, desc->mem, (ulong)desc->mem - (ulong)pool->heap,
