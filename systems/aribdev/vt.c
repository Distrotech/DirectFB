...
#if 0
#define VT_PRINT(x)	{ printf x; }
#else
#define VT_PRINT(x)	{}
#endif

static char SRC_FILE[] = "(+) [DirectFB/systems/aribdev/vt] ";
...
dfb_vt_initialize()
{
     ...
     VT_PRINT(("%s%s: ioctl(VT_GETSTATE)\n", SRC_FILE, __FUNCTION__));
     ...
     if (!dfb_config->vt_switch) {
          ...
          VT_PRINT(("%s%s: ioctl(VT_ACTIVATE)\n", SRC_FILE, __FUNCTION__));
          ...
          VT_PRINT(("%s%s: ioctl(VT_WAITACTIVE)\n", SRC_FILE, __FUNCTION__));
...
dfb_vt_shutdown( bool emergency )
{
     ...
     if (dfb_config->vt_switching) {
          if (ioctl( dfb_vt->fd, VT_SETMODE, &dfb_vt->vt_mode ) < 0)
               D_PERROR( "DirectFB/aribdev/vt: Unable to restore VT mode!!!\n" );
...
dfb_vt_switch( int num )
{
     ...
     D_DEBUG( "DirectFB/aribdev/vt: switching to vt %d...\n", num );

     if (ioctl( dfb_vt->fd0, VT_ACTIVATE, num ) < 0)
          D_PERROR( "DirectFB/aribdev/vt: VT_ACTIVATE failed\n" );
...
vt_thread( DirectThread *thread, void *arg )
{
     ...
     while (true) {
          ...
          D_DEBUG( "DirectFB/aribdev/vt: %s (%d)\n",
          ...
          switch (dfb_vt->vt_sig) {
               ...
               case SIGUSR1:
                    VT_PRINT(("%s%s: SIGUSR1\n", SRC_FILE, __FUNCTION__));
                    if (ioctl( dfb_vt->fd, VT_RELDISP,
                               dfb_core_suspend( NULL ) == DFB_OK ? 1 : 0 ) < 0)
                         D_PERROR( "DirectFB/aribdev/vt: VT_RELDISP failed\n" );

                    break;

               case SIGUSR2:
                    VT_PRINT(("%s%s: SIGUSR2\n", SRC_FILE, __FUNCTION__));
                    ...
                    if (dfb_config->kd_graphics) {
                         if (ioctl( dfb_vt->fd, KDSETMODE, KD_GRAPHICS ) < 0)
                              D_PERROR( "DirectFB/aribdev/vt: KD_GRAPHICS failed!\n" );
                    }
...
vt_init_switching()
{
     ...
     if (dfb_config->kd_graphics) {
          ...
          VT_PRINT(("%s%s: ioctl(KDSETMODE)\n", SRC_FILE, __FUNCTION__));
     }

     if (dfb_config->vt_switch) {
          ioctl( dfb_vt->fd0, TIOCNOTTY, 0 );
          ioctl( dfb_vt->fd, TIOCSCTTY, 0 );
          VT_PRINT(("%s%s: ioctl(TIOCNOTTY,TIOCSCTTY)\n", SRC_FILE, __FUNCTION__));
     }
     ...
     if (dfb_config->vt_switching) {
          ...
          if (sigaction( SIGUSR1, &sig_tty, &dfb_vt->sig_usr1 ) ||
              sigaction( SIGUSR2, &sig_tty, &dfb_vt->sig_usr2 )) {
               D_PERROR( "DirectFB/aribdev/vt: sigaction failed!\n" );
               close( dfb_vt->fd );
               return DFB_INIT;
          }
          VT_PRINT(("%s%s: sigaction(SIGUSR1, SIGUSR2)\n", SRC_FILE, __FUNCTION__));
          ...
          if (ioctl( dfb_vt->fd, VT_SETMODE, &vt ) < 0) {
               D_PERROR( "DirectFB/aribdev/vt: VT_SETMODE failed!\n" );
...
vt_get_fb( int vt )
{
     ...
     VT_PRINT(("%s%s: ioctl(FBIOGET_CON2FBMAP)\n", SRC_FILE, __FUNCTION__));
...
vt_set_fb( int vt, int fb )
{
     ...
     VT_PRINT(("%s%s: ioctl(FBIOPUT_CON2FBMAP)\n", SRC_FILE, __FUNCTION__));
