...
D_DEBUG_DOMAIN( WM_Arib, "WM/Arib", "Arib window manager module" );


DFB_WINDOW_MANAGER( arib )

typedef struct {
     FusionSkirmish mutex_lock;
} WMShared;
...
static void arib_switching_draw_window(
                            CoreWindow       *window,
                            CardState        *state,
                            DFBRegion        *region,
                            DFBRectangle     *src,
                            DFBSwitchingInfo *switching );
             ...
             DFBRegion *region, bool alpha_channel )
{
     ...
     A_TRACE("%s: region(%d,%d,%d,%d) alpha_channel(%d) window->options(%d)\n",
             __FUNCTION__,
             region->x1, region->y1, region->x2, region->y2,
             alpha_channel, config->options);
     ...
     dfb_arib_layer_region_get_temporary_surface( window->primary_region, &surface );

     /* Set blitting source,destination. */
     state->source      = window->surface;
     state->destination = surface? surface : window->primary_region->surface;
...
//                    flags |= DSBLIT_DST_PREMULTIPLY | DSBLIT_DEMULTIPLY;
     ...
     if (switching->active) {
          arib_switching_draw_window( window, state, region, &src, switching );
     }
     else {
...
draw_background( CoreWindowStack *stack, CardState *state, DFBRegion *region )
{
     ...
     A_TRACE("%s: region(%d,%d,%d,%d) mode(%d)\n",
             __FUNCTION__, region->x1, region->y1, region->x2, region->y2, stack->bg.mode);

     /* Initialize destination rectangle. */
     dfb_rectangle_from_region( &dst, region );

     dfb_arib_layer_region_get_temporary_surface( stack->context->primary.region, &surface );

     state->destination = surface? surface : stack->context->primary.region->surface;
     ...
     switch (stack->bg.mode) {
          ...
          case DLBM_DONTCARE: {	/* DFB_ARIB */
               CoreSurface *dest  = state->destination;
               DFBColor     color;

               dfb_get_clear_color( &color, dest->format );
               ...
               DFBSurfaceFlipFlags  flags )
{
     ...
     DFBRegion    modify_region;
     CoreSurface  *tmp_surface = NULL;
     ...
     A_TRACE("%s: update(%d,%d,%d,%d)\n", __FUNCTION__,
              update->x1, update->y1, update->x2, update->y2);

     if (!data->active || !region->surface)
          return;

     dfb_arib_layer_region_get_temporary_surface( region, &tmp_surface );

     dfb_rectangle_from_region( &src_rect, update );

     dfb_arib_layer_region_get_destination_rectangle( region,
                                                      &src_rect, &src_full_rect,
                                                      &dst_rect, &dst_full_rect,
                                                      &modify_rect );

     /* Set destination. */
     state->destination = tmp_surface? tmp_surface : region->surface;
     state->modified   |= SMF_DESTINATION;
     ...
     if (tmp_surface) {
          if (src_full_rect.w == dst_full_rect.w && src_full_rect.h == dst_full_rect.h)
               dfb_arib_layer_region_temporary_to_layer_surface( region, &src_rect, &dst_rect );
          else
               dfb_arib_layer_region_temporary_to_layer_surface( region, &src_full_rect, &dst_full_rect );
     }

     /* Flip the updated region .*/
     dfb_region_from_rectangle( &modify_region, &modify_rect );
     dfb_arib_layer_region_flip_update( region, &modify_region, flags );
}
               ...
               bool                 force_invisible )
{
     ...
     if (region)
	     A_TRACE("%s: region(%d,%d,%d,%d) flags(0x%x) complete(%d) invisible(%d)\n",
     			__FUNCTION__,
     			region->x1, region->y1,region->x2,region->y2,
     			flags, force_complete, force_invisible);
     else
	     A_TRACE("%s: region(NULL) flags(0x%x) complete(%d) invisible(%d)\n",
     			__FUNCTION__, flags, force_complete, force_invisible);
               ...
               int         height )
{
     ...
     D_DEBUG_AT( WM_Arib, "resize_window( %d, %d )\n", width, height );
...
/*== DFB_ARIB Start ==============================================================================*/
static void
arib_switching_draw_window( CoreWindow       *window,
                            CardState        *state,
                            DFBRegion        *region,
                            DFBRectangle     *src,
                            DFBSwitchingInfo *switching )
{
	int          i;
	DFBColor     switching_color;
	DFBRectangle switching_rect;
	DFBRegion    switching_region;
	...
	dfb_get_clear_color( &switching_color, state->destination->format );

	/* still */
	if (switching->attribute) {
		dfb_state_set_color( state, &switching_color );
		dfb_rectangle_from_region( &rectangle, region );
		dfb_gfxcard_fillrectangles( &rectangle, 1, state );
	}
	/* video */
	else {
		dfb_gfxcard_blit( src, region->x1, region->y1, state );
	...
	for (i = 0; i < DFB_SWITCHING_WINDOW_NUM; i++) {

		if (!switching->rectangle[i])
			continue;

		switching_region = *region;

		if (!dfb_region_rectangle_intersect( &switching_region, switching->rectangle[i]))
			continue;

		A_TRACE("%s(%d): (%d) attribute(%d) rectangle(%d,%d,%d,%d)\n",
				__FUNCTION__, window->arib_id, i,
				switching->attribute,
				switching->rectangle[i]->x,switching->rectangle[i]->y,
				switching->rectangle[i]->w,switching->rectangle[i]->h );

		dfb_rectangle_from_region( &switching_rect, &switching_region );
		dfb_state_set_clip( state, &switching_region );

		/* still */
		if (switching->attribute) {
			dfb_gfxcard_blit( &switching_rect, switching_rect.x, switching_rect.y, state );
		}
		/* video */
		else {
			dfb_state_set_color( state, &switching_color );
			dfb_gfxcard_fillrectangles( &switching_rect, 1, state );
		}
	}
}

static void
arib_draw_window( CoreWindow *window,
                  CardState  *state,
                  DFBRegion  *region,
                  bool       alpha_channel )
{
	...
	A_TRACE("%s(%d): region(%d,%d,%d,%d) alpha_channel(%d) window->options(%d)\n",
			__FUNCTION__, window->arib_id,
			region->x1, region->y1, region->x2, region->y2,
			alpha_channel, config->options);
...
static CoreWindow*
get_arib_window( StackData   *data,
                 DFBWindowID arib_id,
                 DFBBoolean  opacity )
{
	int i;
	...
	for (i = 0; i < num; i++) {
		...
		window = fusion_vector_at( &data->windows, i );
		config = &window->config;

		if (arib_id == window->arib_id) {
			if (opacity) {
				if (config->opacity > 0)
					return window;
			}
...
static int
arib_get_src_surface( CoreWindowStack  *stack,
                      StackData        *data,
                      CardState        *state,
                      CoreSurface      **src_surafce,
                      CoreWindowConfig **win_config )
{
	int        i;
	int        num = 0;
	CoreWindow *window;
	...
	D_ASSERT( stack != NULL );
	D_ASSERT( data != NULL );
	D_ASSERT( src_surafce != NULL );
	D_ASSERT( win_config != NULL );
	D_MAGIC_ASSERT( state, CardState );

	/* In batch */
	if (stack->batch_count > 0) {
		return -1;
	}

	/* Find next intersecting window. */
	for (i = DARIBWID_STILLPICTURE_PLANE; i <= DARIBWID_PLANE_MAX; i++) {
		src_surafce[num] = NULL;
		win_config[num]  = NULL;
		window = get_arib_window( data, i, DFB_FALSE );
		if (window) {
			if (VISIBLE_WINDOW( window )) {
				src_surafce[num] = window->surface;
				win_config[num]  = &window->config;
				num++;
			}
		}
	}
...
static void
arib_update_region( CoreWindowStack *stack,
                    StackData       *data,
                    CardState       *state,
                    int             start,
                    int             x1,
                    ...
                    int             y2 )
{
	...
	A_TRACE("%s: region(%d,%d,%d,%d)\n", __FUNCTION__, x1, y1, x2, y2);

	/* Find next intersecting window. */
	while (i >= DARIBWID_STILLPICTURE_PLANE) {
		CoreWindow *window = get_arib_window( data, i, DFB_FALSE );
		if (window) {
			if (VISIBLE_WINDOW( window )) {
				if (dfb_region_intersect( &region,
										  DFB_REGION_VALS_FROM_RECTANGLE( &window->config.bounds )))
	...
	/* Intersecting window found? */
	if (i >= DARIBWID_STILLPICTURE_PLANE) {
		CoreWindow *window = get_arib_window( data, i , DFB_FALSE );
		CoreWindowConfig *config = &window->config;
		...
		if (D_FLAGS_ARE_SET( config->options, DWOP_ALPHACHANNEL | DWOP_OPAQUE_REGION )) {
			...
			if (!dfb_region_region_intersect( &opaque, &region )) {
				arib_update_region( stack, data, state, i-1, x1, y1, x2, y2 );

				arib_draw_window( window, state, &region, true );
			}
			else {
				if ((config->opacity < 0xff) || (config->options & DWOP_COLORKEYING)) {
					/* draw everything below */
					...
					/* left */
					if (opaque.x1 != x1)
						arib_update_region( stack, data, state, i-1, x1, opaque.y1, opaque.x1-1, opaque.y2 );

					/* upper */
					if (opaque.y1 != y1)
						arib_update_region( stack, data, state, i-1, x1, y1, x2, opaque.y1-1 );

					/* right */
					if (opaque.x2 != x2)
						arib_update_region( stack, data, state, i-1, opaque.x2+1, opaque.y1, x2, opaque.y2 );

					/* lower */
					if (opaque.y2 != y2)
						arib_update_region( stack, data, state, i-1, x1, opaque.y2+1, x2, y2 );
				}

				/* left */
				if (opaque.x1 != region.x1) {
					DFBRegion r = { region.x1, opaque.y1, opaque.x1 - 1, opaque.y2 };
					arib_draw_window( window, state, &r, true );
				}
				...
				/* inner */
				arib_draw_window( window, state, &opaque, true );
			}
		}
		else {
			if (TRANSLUCENT_WINDOW( window )) {
				...
				/* left */
				if (region.x1 != x1)
					arib_update_region( stack, data, state, i-1, x1, region.y1, region.x1-1, region.y2 );

				/* upper */
				if (region.y1 != y1)
					arib_update_region( stack, data, state, i-1, x1, y1, x2, region.y1-1 );

				/* right */
				if (region.x2 != x2)
					arib_update_region( stack, data, state, i-1, region.x2+1, region.y1, x2, region.y2 );

				/* lower */
				if (region.y2 != y2)
					arib_update_region( stack, data, state, i-1, x1, region.y2+1, x2, y2 );
			}
			arib_draw_window( window, state, &region, true );
		}
	}
...
static void
arib_repaint_stack( CoreWindowStack     *stack,
                    StackData           *data,
                    CoreLayerRegion     *region,
                    const DFBRegion     *update,
                    DFBSurfaceFlipFlags flags )
{
	...
	int              num;
	CoreSurface      *tmp_surface = NULL;
	CoreSurface      *src_surfaces[DARIBWID_PLANE_MAX];
	CoreWindowConfig *win_config[DARIBWID_PLANE_MAX];
	...
	/* Get Arib Surface & Window Config */
	num = arib_get_src_surface( stack, data, state, src_surfaces, win_config );
	if (num < 0) {
		A_TRACE("%s: arib_get_src_surface In Batch\n", __FUNCTION__);
		return;
	}

	/* Set windowstack region. */
	DFB_RECTANGLE_SET( &stack_rect, 0, 0, stack->width, stack->height );
	src_rect = stack->modify_rect;

	dfb_rectangle_intersect( &src_rect, &stack_rect );
	...
	A_TRACE("%s: num(%d) modify(%d,%d,%d,%d) (%d,%d,%d,%d)->(%d,%d,%d,%d)\n",
			__FUNCTION__, num,
	...
	if (tmp_surface) {
		if (num) {
			dfb_gfxcard_multi_blit( num,
									&src_rect, src_surfaces,
									&src_rect, tmp_surface,
									state, (void **)win_config );
			...
			dfb_arib_layer_region_clear_temporary_surface( region, &src_full_rect );
			dfb_arib_layer_region_temporary_to_layer_surface( region, &src_full_rect, &dst_full_rect );
		}
	}
	else {
		if (num) {
			dfb_gfxcard_multi_blit( num,
									&src_rect, src_surfaces,
									&dst_rect, region->surface,
									state, (void **)win_config );
		}
		else {
			DFBColor color;

			dfb_get_clear_color( &color, region->surface->format );
			dfb_state_set_color( state, &color );

			state->destination = region->surface;
			state->modified   |= SMF_DESTINATION;
			/* Simply fill the background. */
			dfb_gfxcard_fillrectangles( &src_full_rect, 1, state );
			state->destination = NULL;
			state->modified   |= SMF_DESTINATION;
		}
	}
	...
	dfb_region_from_rectangle( &modify_region, &modify_rect );
	dfb_arib_layer_region_flip_update( region, &modify_region, flags );
	DFB_RECTANGLE_SET( &stack->modify_rect, 0, 0, 0, 0 );
}

static DFBResult
arib_update_stack( CoreWindowStack     *stack,
                   StackData           *data,
                   const DFBRegion     *region,
                   DFBSurfaceFlipFlags flags )
{
	...
	dfb_rectangle_from_region( &rect, &area );
	dfb_rectangle_union( &stack->modify_rect, &rect );

	if (!stack->batch_count) {
		arib_repaint_stack( stack, data, primary, &area, 0 );
	}
	/* Unref primary region. */
	dfb_layer_region_unref( primary );
...
static DFBResult
arib_update_window( CoreWindow          *window,
                    WindowData          *window_data,
                    const DFBRegion     *region,
                    DFBSurfaceFlipFlags flags,
                    bool                force_complete,
                    bool                force_invisible )
{
	...
	if (!dfb_unsafe_region_intersect( &area, 0, 0, surface->width - 1, surface->height - 1 ))
		return DFB_OK;
	...
	if (region)
		A_TRACE("%s(%d): batch_count(%d) region(%d,%d,%d,%d)\n", __FUNCTION__,
				window->arib_id, stack->batch_count,
				region->x1, region->y1, region->x2, region->y2);
	else
		A_TRACE("%s(%d): batch_count(%d) region(NULL) \n", __FUNCTION__,
				window->arib_id, stack->batch_count);

	if (!stack->batch_count) {
		arib_repaint_stack( stack, data, window->primary_region, &area, flags );
	}
	return DFB_OK;
}

static void
arib_set_opacity( CoreWindow *window,
                  WindowData *window_data,
                  __u8       opacity )
{
     __u8            old;
     ...
     A_TRACE("%s(%d): (%d)\n", __FUNCTION__, window->arib_id, opacity);
     ...
     if (old != opacity) {
          ...
          if (window->region) {
               ...
               arib_update_window( window, window_data, NULL, DSFLIP_NONE, false, true );
          }
...
static DFBResult
arib_set_switching_region( CoreWindow   *window,
                           WindowData   *window_data,
                           DFBRectangle *rect1,
                           DFBRectangle *rect2,
                           DFBRectangle *rect3,
                           ...
                           DFBBoolean   attribute )
{
	...
	DFBRegion        region;
	int              i;
	DFBRectangle     *rect[DFB_SWITCHING_WINDOW_NUM];
	CoreWindowConfig *config;
	DFBSwitchingInfo *switching;
	...
	switching->active    = DFB_FALSE;
	switching->attribute = attribute;

	for (i = 0; i < DFB_SWITCHING_WINDOW_NUM; i++) {
		if (switching->rectangle[i]) {
			SHFREE(switching->rectangle[i]);
			switching->rectangle[i] = NULL;
		}
		if (rect[i] == NULL) {
			switching->rectangle[i] = NULL;
			continue;
		}
		if (rect[i]->w <= 0 || rect[i]->h <= 0) {
			switching->rectangle[i] = NULL;
			continue;
		}
		dfb_region_from_rectangle( &region, rect[i] );
		if (!dfb_unsafe_region_intersect( &region, 0, 0, surface->width - 1, surface->height - 1 )) {
			switching->rectangle[i] = NULL;
			continue;
		}
		switching->rectangle[i] = (DFBRectangle *)SHCALLOC( 1, sizeof(DFBRectangle) );
		if (switching->rectangle[i]) {
			dfb_rectangle_from_region( switching->rectangle[i], &region );
			switching->rectangle[i]->x += config->bounds.x;
			switching->rectangle[i]->y += config->bounds.y;
			switching->active = DFB_TRUE;

			A_TRACE("%s(%d): num(%d) attribute(%d) rectangle(%d,%d,%d,%d)\n", __FUNCTION__, window->arib_id,
					i+1, switching->attribute,
					switching->rectangle[i]->x,switching->rectangle[i]->y,switching->rectangle[i]->w,switching->rectangle[i]->h );
		}
	}
	return DFB_OK;
}
...
static DFBResult
arib_check_window_id( CoreWindowStack *stack,
                      StackData       *data,
                      DFBWindowID     arib_id,
                      CoreWindow      **ret_window)
{
	...
	*ret_window = get_arib_window( data, arib_id, DFB_FALSE );
	if (*ret_window) {
		return DFB_BUSY;
	}
...
/**************************************************************************************************/
static void
wm_semaphore_init( WMData *wm_data )
{
	fusion_skirmish_init( &wm_data->shared->mutex_lock, "WM" );
}

static void
wm_semaphore_lock( WMData *wm_data )
{
	fusion_skirmish_prevail( &wm_data->shared->mutex_lock );
}

static void
wm_semaphore_unlock( WMData *wm_data )
{
	fusion_skirmish_dismiss( &wm_data->shared->mutex_lock );
}

static void
wm_semaphore_destroy( WMData *wm_data )
{
	fusion_skirmish_destroy( &wm_data->shared->mutex_lock );
}
...
wm_get_info( CoreWMInfo *info )
{
     ...
     info->version.binary = 0;

     snprintf( info->name, DFB_CORE_WM_INFO_NAME_LENGTH, "Arib" );
     snprintf( info->vendor, DFB_CORE_WM_INFO_VENDOR_LENGTH, "Mitsubishi Electric Corp" );
                 ...
                 DFBSurfaceFlipFlags  flags )
{
     ...
     ret = update_stack( stack, data, region, flags );
                  ...
                  DFBSurfaceFlipFlags  flags )
{
     ...
     ret = update_window( window, window_data, region, flags, false, false );
...
/*== DFB_ARIB Start ==============================================================================*/
static DFBResult
wm_arib_set_window_config( CoreWindow             *window,
                           void                   *wm_data,
                           void                   *window_data,
                           const CoreWindowConfig *config,
                           CoreWindowConfigFlags  flags )
{
	...
	if (flags & CWCF_OPACITY && !config->opacity)
		arib_set_opacity( window, window_data, config->opacity );
...
static DFBResult
wm_arib_update_stack( CoreWindowStack     *stack,
                      void                *wm_data,
                      void                *stack_data,
                      const DFBRegion     *region,
                      DFBSurfaceFlipFlags flags )
{
	...
	ret = arib_update_stack( stack, data, region, flags );
...
static DFBResult
wm_arib_update_window( CoreWindow          *window,
                       void                *wm_data,
                       void                *window_data,
                       const DFBRegion     *region,
                       DFBSurfaceFlipFlags flags )
{
	...
	ret = arib_update_window( window, window_data, region, flags, false, false );
...
static DFBResult
wm_arib_set_switching_region( CoreWindow   *window,
                              void         *wm_data,
                              void         *window_data,
                              DFBRectangle *rect1,
                              DFBRectangle *rect2,
                              ...
                              DFBBoolean   attribute )
{
	...
	return arib_set_switching_region( window, window_data, rect1, rect2, rect3, rect4, attribute );
}

static DFBResult
wm_arib_check_window_id( CoreWindowStack *stack,
                         void            *stack_data,
                         void            *wm_data,
                         DFBWindowID     arib_id,
                         CoreWindow      **ret_window)
{
	...
	return arib_check_window_id( stack, stack_data, arib_id, ret_window );
}

static DFBResult
wm_arib_set_active( CoreWindowStack *stack,
                    void            *wm_data,
                    void            *stack_data,
                    bool             active )
{
	...
	if (active)
		return dfb_arib_windowstack_repaint_all( stack );
